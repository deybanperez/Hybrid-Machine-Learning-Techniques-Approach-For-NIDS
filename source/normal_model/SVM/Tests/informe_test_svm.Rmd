...
```
La media de aciertos es de **99.19%**. Una tasa de aciertos bastante alta que demuestra que las máquinas de soporte vectorial pueden tener un buen desempeño en la tarea de detección de intrusos en redes de computadoras. A continuación se creará una matriz de confusión para ilustrar de manera gráfica el desempeño del modelo.
...
```
La mejor tasa de aciertos fue de **99.37%**  y la tasa de fallos corresponde a su complemento y corresponde al **0.63%**. Ahora veamos la eficacia por etiqueta a la hora de clasificar. La salida corresponde a un vector donde cada posición correspode a **DoS**, **normal**, **Probing**, **R2L**, **U2R**. Esto debido a que las etiquetas se ordenaron alfabeticamente.

```{r}
AccuracyPerLabel(confusion.matrix, list.results$best_testing_set)
```
Se observa que la eficacia con las clases **DoS**, **normal**, **Probing** y **R2L** está por encima del 92.9%, mientras que para **U2R** la tasa de aciertos es de sólo 33.3%. Este comportamiento se debe a que para **U2R** la cantidad de ejemplos para el entrenamiento son menores y por eso el algoritmo no es capaz de generalizar de la mejor manera, en otras palabras, se espera que con mayor cantidad de información las predicciones para esta clase mejore.
...
```
En la curva se puede observar cómo el modelo comete muchos errores con altas tasas de probabilidad, es decir, el modelo asegura que un registro pertenece a una clase con una probabilidad bastante alta y comete muchos errores. La mejor medida de acierto se alcanza con valores de probabilidad intermedios.
...
### Añadiendo el segundo nivel de clasificación de K-Medias
El modelo anterior presentó 25 **falsos negativos**, esto quiere decir que se clasificaron 25 registros cómo **normal** que en verdad pertenecían a la etiqueta **Attack**. Se tomarán todas aquellas predicciones realizadas por el modelo de máquina de soporte vectorial con la etiqueta **normal** y se utilizará **k-medias** para tratar de separar esos 25 falsos negativos de los registros normales. Se utilizará k-medias con 2 clusters debido a que se verificó en el **informe de kmedias** que la varianza acumulada llegaba a su mejor representación en el codo de jambu con 2 clusters en vez de con 5.
...
```
Podemos observar cómo despues de tomar las etiquetas clasificadas cómo normales, se extrayeron los 25 falsos negativos y las 4417 etiquetas restantes corresponden a las verdaderas etiquetas normales.
...
```
Podemos ver que 0 de los 25 ataques fueron detectados, volvió a tener 25 falsos begativos e incrementó en una unidad la cantidad de falsos positivos. A simple vista no parece que aplicar K-medias sirviera de mucho, sin embargo, se espera que este rendimiento mejore conforme haya mayor cantidad de ataques. Ahora calcularemos las tasas de acierto y de error.
...
```
La tasa de aciertos es de 99.41% y la tasa de error de 0.59%, aparentemente tiene un buen resultado, también hay que considerar que las clases están desbalanceadas en este punto. Ahora veamos la eficacia por etiqueta. Recordemos que la salidad es un vector que corresponde a **Attack** y **normal**.

```{r}
AccuracyPerLabel(confusion.matrix.kmeans.model, kmeans.set)
```
Se obtuvo 99.98% de acierto en la etiqueta normal y 0% en ataques, este resultado no es bueno, debido a que que el objetivo es detectar ataques y los 25 ataques fueron clasificados de mala manera. Ahora calcularemos las medidas de rendimiento binarias **Sensitivity**, **Especificity** y **Precision**.
...
1. El algoritmo de redes máquinas de soporte vectorial tiene un desempeño excelente, con bajas tasas de falsos positivos, falsos negativos y alta tasa de aciertos.
...
3. La combinación de ambos modelos es igual a sólo utilizar máquinas de soporte vectorial.